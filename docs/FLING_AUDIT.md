...
1. **Drive rooms**: `DriveVideoPlayer.didChangeAppLifecycleState` on `paused` schedules a **300ms-delayed** `_handoffToBackgroundAudio()` (skipped if PiP is active), which pauses the local `VideoPlayerController` and then calls `BackgroundAudioHandler.loadAndPlay(url, headers, initialPosition, ...)` — a **fresh** `just_audio` `setAudioSource()` + `play()` against a **second, separate player instance**, re-streaming from `/drive/stream/:fileId` from scratch rather than continuing a warm source. That 300ms delay plus a cold network-source setup is a real, audible gap — this is the closest match to the reported symptom, and it is fixable (attach the background player's source proactively / eliminate the delay / consider a single shared player rather than a handoff, if the platform allows it).
...
1. **iOS: no `UIBackgroundModes` audio capability declared** — real background audio is currently impossible on iOS regardless of provider. (§5.2)
2. **Drive background-audio handoff has a built-in 300ms delay plus a cold source reload** — the most concrete, fixable cause of an audible gap on lock/background for Drive rooms. (§5.1)
...
- Fix the Drive handoff delay/cold-reload (§5.1) and the iOS background-audio capability gap (§5.2) as part of the `PlaybackSessionManager` work, since both are squarely inside that component's responsibility.
